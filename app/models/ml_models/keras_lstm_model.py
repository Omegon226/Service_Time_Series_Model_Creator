...
class KerasLSTMModel(MLModelBase):
        ...
        for iteration in range(len(hidden_layers_size)):
            if iteration == 0:
                if iteration != len(hidden_layers_size) - 1:
                    try:
                        hidden_layers += [keras.layers.LSTM(hidden_layers_size[iteration],
                                                                 return_sequences=True,
                                                                 activation=hidden_layers_activation[iteration],
                                                                 name=f"hidden_{iteration + 1}_lstm")(input_layer)]
                ...
                if iteration != len(hidden_layers_size) - 1:
                    try:
                        hidden_layers += [keras.layers.LSTM(hidden_layers_size[iteration],
                                                                 return_sequences=True,
                                                                 activation=hidden_layers_activation[iteration],
                                                                 name=f"hidden_{iteration + 1}_lstm")(hidden_layers[-1])]
                    except (ValueError, IndexError) as error:
                        hidden_layers += [keras.layers.LSTM(hidden_layers_size[iteration],
                                                                 return_sequences=True,
                                                                 activation="relu",
