...
class KerasRNNModel(MLModelBase):
    def __init__(self, hidden_layers_size: list, hidden_layers_activation: list, amount_of_params: int,
        ...
        for iteration in range(len(hidden_layers_size)):
            if iteration == 0:
                if iteration != len(hidden_layers_size) - 1:
                    try:
                        hidden_layers += [keras.layers.SimpleRNN(hidden_layers_size[iteration],
                                                                 return_sequences=True,
                                                                 activation=hidden_layers_activation[iteration],
                                                                 name=f"hidden_{iteration + 1}_rnn")(input_layer)]
                ...
                if iteration != len(hidden_layers_size) - 1:
                    try:
                        hidden_layers += [keras.layers.SimpleRNN(hidden_layers_size[iteration],
                                                                 return_sequences=True,
                                                                 activation=hidden_layers_activation[iteration],
                                                                 name=f"hidden_{iteration + 1}_rnn")(hidden_layers[-1])]
                    except (ValueError, IndexError) as error:
                        hidden_layers += [keras.layers.SimpleRNN(hidden_layers_size[iteration],
                                                                 return_sequences=True,
                                                                 activation="relu",
