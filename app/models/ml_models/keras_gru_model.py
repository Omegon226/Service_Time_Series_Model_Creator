...
class KerasGRUModel(MLModelBase):
        ...
        for iteration in range(len(hidden_layers_size)):
            if iteration == 0:
                if iteration != len(hidden_layers_size) - 1:
                    try:
                        hidden_layers += [keras.layers.GRU(hidden_layers_size[iteration],
                                                                 return_sequences=True,
                                                                 activation=hidden_layers_activation[iteration],
                                                                 name=f"hidden_{iteration + 1}_gru")(input_layer)]
                ...
                if iteration != len(hidden_layers_size) - 1:
                    try:
                        hidden_layers += [keras.layers.GRU(hidden_layers_size[iteration],
                                                                 return_sequences=True,
                                                                 activation=hidden_layers_activation[iteration],
                                                                 name=f"hidden_{iteration + 1}_gru")(hidden_layers[-1])]
                    except (ValueError, IndexError) as error:
                        hidden_layers += [keras.layers.GRU(hidden_layers_size[iteration],
                                                                 return_sequences=True,
                                                                 activation="relu",
